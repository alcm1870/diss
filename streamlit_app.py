...
def checkpoint_down():
    ...
    # 创建ObsClient实例
    obsClient = ObsClient(
        access_key_id='R6MYCJPEWKGWPBBGHRBU',  # 刚刚下载csv文件里面的Access Key Id
        secret_access_key='2dUK3uFkTofXJLlJgNqqqKypgrjYbKYT6SU1KwJc',  # 刚刚下载csv文件里面的Secret Access Key
        server='https://obs.cn-southwest-2.myhuaweicloud.com'  # 这里的访问域名就是我们在桶的基本信息那里记下的东西
    )
    # 调用getObject接口下载桶内对象
    # 本地路径记得加上文件名和后缀  比如F:/test_picture/1.jpg
    os.makedirs(f"CATS_LOSS", exist_ok=True)
    os.makedirs(f"BSCN_LOSS2", exist_ok=True)
    resp = obsClient.getObject('model-pth', 'checkpoints/CATS_LOSS/3/3_model.pth', downloadPath='./CATS_LOSS/3_model.pth')
    resp1=obsClient.getObject('model-pth', 'checkpoints/BSCN_LOSS2/3/3_model.pth', downloadPath='./BSCN_LOSS2/3_model.pth')
    if resp.status < 300:
...
def main():
    ...
    if uploaded_file is not None:
        ...
        model = DexiNed()
        checkpoint_path=f'{add_loss}/3_model.pth'
